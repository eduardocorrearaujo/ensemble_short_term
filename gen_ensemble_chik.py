...
def get_preds(state, model, week, disease): 
    ...
    df = pd.read_csv(f'forecast_tables/for_{model}_{disease}_{week}_{state}.csv.gz')
...
def get_all_preds(state, week, disease):
    ...
    df_preds =pd.concat([df_pred2, df_pred3])
    
    return df_preds.rename(columns = {'lower': 'lower_95', 
                                         'upper': 'upper_95'})
    
def get_ensemble(state, max_date, week, disease):
    ...
    weights = np.array([1/2, 1/2])#weights_crps['weights']
...
if __name__ == '__main__':
    ...
    for state in states_BR:
        print(state)
        df_crps_ , weights_ = get_ensemble(state, max_date, for_week, disease=disease)
    ...
    df_crps.to_csv(f'forecast_tables/for_ensemble_{year}_{for_week}_{disease}.csv', index = False)
